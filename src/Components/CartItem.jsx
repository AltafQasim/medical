...
const CartItem = ({
  ...
  const handleCount = (count) => {
    ...
    dispatch(
      updateItem({ id, title, price, rating, desc, images, quantity: count })
    );
  };
      ...
      <div className="w-full flex xs-mx:p-1 p-2 gap-2 items-center flex-wrap justify-between">
        <div className="flex gap-2 items-center">
          <img
            ...
            src={`../../Images/products/${title + images[0]}`}
          />
          <div className="flex flex-col gap-2 ">
            ...
            <span className="text-sm xs-mx:text-xs text-gray-700 font-semibold">
        ...
        <div className="flex xs-mx:w-full h-full  xs:flex-col items-center justify-between xs-mx:gap-2 gap-3">
          ...
          <div
            className={`flex ${
              cart ? "gap-4" : "gap-1"
            } rounded-xl xsm-mx:gap-2 text-gray-600 text-sm font-semibold items-center px-3 py-1.5 bg-gray-100 w-fit`}
          >
            {cart ? (
              <MinusIcon
          ...
          {cart && (
            <TrashIcon
              ...
              className="w-6 h-6 text-gray-600 hover:text-red-500 m-2 xs-mx:m-0 cursor-pointer"
            />
          )}
        </div>
      </div>
